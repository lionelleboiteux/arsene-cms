 ...
 * Same labels/urls as `fc-shared/nav.js`'s own `SOCIALS` array; icons are
 * separate static files (`public-site/public/assets/social-*.png`), not
 * inlined base64 — an earlier inlined attempt silently broke (a data: URI
 * mangled in transit renders as nothing, with the `alt` text as the only
 * visible fallback), and a real file the browser fetches independently is
 * both more robust and lets Cloudflare cache it like any other asset.
 */
const HOME_SOCIALS: { label: string; url: string; icon: string }[] = [
  { label: 'Bluesky', url: 'https://bsky.app/profile/fantasycoachfr.bsky.social', icon: '/assets/social-bluesky.png' },
  { label: 'Mastodon', url: 'https://mastodon.social/@FantasyCoachFR', icon: '/assets/social-mastodon.png' },
